...
echo "update libraries and dependencies and install java, mvn and scala"
sleep 2
...
echo "Generating SideChain Keys..."
...
echo "Preparing zen.conf..."

read -e -p "rpcuser : " USER
echo "you can use this random password " && echo `openssl rand -base64 32`
read -e -p "rpcpassword : " PASS

echo 'rpcuser="$USER"' >> ~/.zen/zen.conf
echo 'rpcpassword="$PASS"' >> ~/.zen/zen.conf
...
echo "starting zend_oo..."

zend -websocket
sleep 10
read -e -p "How many Blocks to generate for Mainchain? : " BLOCKS
zen-cli generate $BLOCKS
sleep 15

read -e -p "How many Epochs for Sidechain? : " EPOCHS
read -e -p "How many ZEN coins to send? : " ZENC

CREATE=`zen-cli sc_create $EPOCHS "$GKPUBLIC" $ZENC "$GPIVK" "$GVKPUBLIC" "$GPIGENSYS"` && echo CREATE = $CREATE
sleep 10
...
echo "preparing SimpleApp configuration file: 'my-sidechain.conf'..."
